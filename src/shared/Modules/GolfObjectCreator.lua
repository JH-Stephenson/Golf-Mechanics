local GolfObjectCreator = {}

function GolfObjectCreator.CreateGolfBall(Player, GolfBall, GolfTee)
    task.spawn(function()
...
function GolfObjectCreator.CreateGolfTee(Player, GolfTee)
    if not GolfTee then
        local RaycastingModule = require(script.Parent.RaycastingModule)
        local RaycastingResult = RaycastingModule.FindPlayersGround(Player)
